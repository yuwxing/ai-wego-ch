...
export default function IsometricTeacher({ animation = 'idle', animSpeed = 1, visible = true, walkDir }: Props) {
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      ...
      <primitive object={joints.body}>
        {/* Blazer */}
        <Box pos={[0, 0, 0]} size={[PX * 3.5, PX * 3.5, PX * 2]} color="#6366f1" />
        {/* White shirt collar */}
        <Box pos={[0, PX * 1.6, PX * 1.1]} size={[PX * 1.2, PX * 0.5, PX * 0.3]} color="#f1f5f9" />
        {/* Skirt */}
        <Box pos={[0, -PX * 1.5, 0]} size={[PX * 3.5, PX * 1.5, PX * 2]} color="#4f46e5" />
        {/* Belt */}
        <Box pos={[0, -PX * 0.3, 0]} size={[PX * 3.8, PX * 0.4, PX * 2.2]} color="#1e1b4b" />
      </primitive>

      {/* Head */}
      <primitive object={joints.head}>
        {/* Face */}
        <Box pos={[0, PX * 1, 0]} size={[PX * 2.8, PX * 2.8, PX * 2.2]} color="#fcd9b6" />
        {/* Hair – top */}
        <Box pos={[0, PX * 2.2, 0]} size={[PX * 3, PX * 0.8, PX * 2.4]} color="#1c1917" />
        {/* Hair – front bangs */}
        <Box pos={[0, PX * 1.6, PX * 1.2]} size={[PX * 2.4, PX * 0.6, PX * 0.3]} color="#292524" />
        <Box pos={[-PX * 0.6, PX * 1.2, PX * 1.2]} size={[PX * 0.5, PX * 0.8, PX * 0.3]} color="#292524" />
        <Box pos={[PX * 0.6, PX * 1.2, PX * 1.2]} size={[PX * 0.5, PX * 0.8, PX * 0.3]} color="#292524" />
        {/* Hair – left long */}
        <Box pos={[-PX * 1.8, PX * 0, 0]} size={[PX * 0.5, PX * 2.5, PX * 1.2]} color="#1c1917" />
        <Box pos={[-PX * 1.8, -PX * 1.5, 0]} size={[PX * 0.5, PX * 1.5, PX * 0.6]} color="#292524" />
        {/* Hair – right long */}
        <Box pos={[PX * 1.8, PX * 0, 0]} size={[PX * 0.5, PX * 2.5, PX * 1.2]} color="#1c1917" />
        <Box pos={[PX * 1.8, -PX * 1.5, 0]} size={[PX * 0.5, PX * 1.5, PX * 0.6]} color="#292524" />
        {/* Hair – back */}
        <Box pos={[0, PX * 0.5, -PX * 1.3]} size={[PX * 2.6, PX * 2.5, PX * 0.5]} color="#1c1917" />
        {/* Eyes */}
        <Box pos={[-PX * 0.6, PX * 1.2, PX * 1.2]} size={[PX * 0.4, PX * 0.4, PX * 0.2]} color="#1e293b" />
        <Box pos={[PX * 0.6, PX * 1.2, PX * 1.2]} size={[PX * 0.4, PX * 0.4, PX * 0.2]} color="#1e293b" />
        {/* Mouth (smile) */}
        <Box pos={[0, PX * 0.6, PX * 1.2]} size={[PX * 0.8, PX * 0.2, PX * 0.2]} color="#ef4444" />
        {/* Glasses */}
        <Box pos={[-PX * 0.6, PX * 1.2, PX * 1.3]} size={[PX * 0.8, PX * 0.6, PX * 0.1]} color="#38bdf8" emissive="#38bdf8" />
        <Box pos={[PX * 0.6, PX * 1.2, PX * 1.3]} size={[PX * 0.8, PX * 0.6, PX * 0.1]} color="#38bdf8" emissive="#38bdf8" />
        <Box pos={[0, PX * 1.2, PX * 1.3]} size={[PX * 0.4, PX * 0.1, PX * 0.1]} color="#94a3b8" />
        {/* Hair accessory – little flower */}
        <Box pos={[PX * 1.2, PX * 1.8, -PX * 0.3]} size={[PX * 0.5, PX * 0.5, PX * 0.3]} color="#f472b6" emissive="#f472b6" />
      </primitive>

      {/* Arms */}
      <primitive object={joints.leftArm}>
        <Box pos={[0, 0, 0]} size={[PX * 0.7, PX * 2.8, PX * 0.7]} color="#6366f1" />
        <Box pos={[0, -PX * 1.6, 0]} size={[PX * 0.6, PX * 0.6, PX * 0.6]} color="#fcd9b6" />
      </primitive>
      ...
      <primitive object={joints.leftLeg}>
        <Box pos={[0, -PX * 0.8, 0]} size={[PX * 0.6, PX * 1.6, PX * 0.6]} color="#1e1b4b" />
        {/* Shoe */}
        <Box pos={[0, -PX * 1.8, PX * 0.2]} size={[PX * 0.7, PX * 0.5, PX * 1.0]} color="#0f172a" />
      </primitive>
