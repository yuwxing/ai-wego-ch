...
type UserRole = 'student' | 'teacher' | 'principal' | 'parent' | 'professional';
...
export const RegisterPage: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    avatarData.id = Math.random().toString(36).substring(2, 10)
    const list = JSON.parse(localStorage.getItem('digitalAvatars') || '[]')
    list.push(avatarData)
    localStorage.setItem('digitalAvatars', JSON.stringify(list))
    localStorage.setItem('activeAvatarId', avatarData.id)
    ...
    try {
      ...
      for (let a = 0; a < 3; a++) {
        try {
          ...
          lastErr = e;
          if (a < 2) await new Promise(r => setTimeout(r, 1000));
        }
      }
      if (lastErr && (!users || !users.length)) throw lastErr;
      ...
      <div className="flex items-center gap-2 mb-6">
        {progressSteps.map((s, i) => (
          <React.Fragment key={s}>
            <div className={`flex items-center gap-1.5 ${i <= idx ? 'text-emerald-500' : 'text-slate-300'}`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${i <= idx ? 'bg-gradient-to-r from-emerald-400 to-teal-400 text-white' : 'bg-slate-200 text-slate-400'}`}>
                {i < idx ? '✓' : i + 1}
              </div>
              <span className="text-xs font-medium hidden sm:inline">{s}</span>
            </div>
            {i < progressSteps.length - 1 && <div className={`flex-1 h-0.5 ${i < idx ? 'bg-gradient-to-r from-emerald-300 to-teal-300' : 'bg-slate-200'}`} />}
          </React.Fragment>
        ))}
      </div>
    );
      ...
      <div className="page-wrapper py-8 px-4">
        <div className="max-w-lg mx-auto space-y-6 relative z-10">
          <div className="glass-card !p-8 text-center rounded-2xl">
            <div className="w-20 h-20 mx-auto mb-6 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-full flex items-center justify-center shadow-md">
            ...
            {roleConfig && generated && (
              <div className={`${roleConfig.lightBg} rounded-xl p-5 mb-6 text-left border border-white/50`}>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-slate-600 bg-white/80 rounded-lg px-3 py-2 border border-slate-100">{avatar.goal}</p>
                </div>
              </div>
            )}
            <div className="bg-amber-50 rounded-xl p-4 mb-6 flex items-center justify-center gap-4 border border-amber-100/50">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => navigate(`/avatar-chat`)} className="flex-1 py-3 btn-gradient-primary rounded-xl font-medium flex items-center justify-center gap-2"><MessageCircle className="w-4 h-4" /> 和TA聊天</button>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="page-wrapper py-6 px-4 min-h-screen">
      <div className="max-w-2xl mx-auto space-y-5 relative z-10">
        <button onClick={() => navigate('/')} className="inline-flex items-center gap-2 text-slate-500 hover:text-emerald-600 transition-colors text-sm">
        ...
        {activeTab === 'human' && step === 0 && (
          <div className="text-center">
            <h1 className="text-3xl font-bold text-slate-900 mb-3">
              创建你的 <span className="gradient-text-primary">AI分身</span>
            </h1>
        ...
        <div className="flex gap-1.5 p-1.5 bg-slate-100/80 rounded-2xl backdrop-blur-sm">
          <button onClick={() => { setActiveTab('human'); setStep(0); setError(null); }} className={`flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl font-medium transition-all text-sm ${activeTab === 'human' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
            <User className="w-4 h-4" /> 创建分身
          </button>
          <button onClick={() => { setActiveTab('login'); setError(null); }} className={`flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl font-medium transition-all text-sm ${activeTab === 'login' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                ...
                <input type="text" value={form.email} onChange={(e) => setForm(prev => ({ ...prev, email: e.target.value }))}
                  className="w-full px-4 py-3 bg-white/70 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-300 focus:border-transparent outline-none transition-all text-sm" placeholder="输入用户名或邮箱" />
              </div>
              <button type="submit" disabled={loading}
                className="w-full py-3.5 btn-gradient-primary rounded-xl font-bold text-base hover:shadow-lg disabled:opacity-50">{loading ? '登录中...' : '登录'}</button>
              <div className="text-center text-sm text-slate-400">还没有账号？<button type="button" onClick={() => { setActiveTab('human'); setStep(0); setError(null); }} className="text-emerald-600 hover:text-emerald-700 font-medium">去创建分身</button></div>
            </form>
          </div>
        ) : (
          <>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={form.username} onChange={(e) => setForm(p => ({ ...p, username: e.target.value }))}
                      className="w-full px-4 py-3 bg-white/70 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-300 focus:border-transparent outline-none transition-all text-sm" placeholder="给自己取个名字" />
                  </div>
                  <div>
                    ...
                    <input type="email" value={form.email} onChange={(e) => setForm(p => ({ ...p, email: e.target.value }))}
                      className="w-full px-4 py-3 bg-white/70 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-300 focus:border-transparent outline-none transition-all text-sm" placeholder="没有可不填" />
                  </div>
                  <div>
                    ...
                    <input type="password" value={form.password} onChange={(e) => setForm(p => ({ ...p, password: e.target.value }))}
                      className="w-full px-4 py-3 bg-white/70 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-300 focus:border-transparent outline-none transition-all text-sm" placeholder="设置密码（可选）" />
                  </div>
                  <button type="button" onClick={() => { setError(null); if (!validateRegister()) return; setStep(1); }}
                    className="w-full py-3.5 btn-gradient-primary rounded-xl font-bold text-base hover:shadow-lg transition-all">下一步：选择身份</button>
                </div>
              </div>
            )}
              ...
              <div className="glass-card !p-6 rounded-2xl">
                ...
                <div className="space-y-3">
                  {roles.map(r => (
                    <button key={r.id} type="button" onClick={() => { setSelectedRole(r.id); setAvatar(prev => ({ ...prev, name: r.companionTitle })); }}
                      className={`w-full p-5 rounded-2xl text-left transition-all border-2 ${selectedRole === r.id ? `${r.lightBg} border-current` : 'bg-white/50 border-transparent hover:border-slate-200'}`}
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        {selectedRole === r.id && (
                          <div className={`w-6 h-6 rounded-full bg-gradient-to-br ${r.gradient} flex items-center justify-center shadow-sm`}>
                ...
                <div className="flex gap-3 mt-5">
                  ...
                  <button type="button" onClick={() => { if (!selectedRole) { setError('请选择你的身份'); return; } setStep(2); }}
                    className="flex-1 py-3.5 btn-gradient-primary rounded-xl font-bold hover:shadow-lg transition-all">下一步：创建分身</button>
                </div>
              </div>
            )}
            ...
            {step === 2 && roleConfig && (
              <div className="glass-card !p-6 rounded-2xl">
                ...
                <div className={`${roleConfig.lightBg} rounded-2xl p-4 mb-5 border border-white/50`}>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={avatar.name} onChange={(e) => setAvatar(p => ({ ...p, name: e.target.value }))}
                      className="w-full px-4 py-3 bg-white/70 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-300 focus:border-transparent outline-none transition-all text-sm" placeholder={`如：${roleConfig.companionTitle}`} />
                  </div>
                  <div>
                    ...
                    <textarea value={avatar.goal} onChange={(e) => setAvatar(p => ({ ...p, goal: e.target.value }))}
                      className="w-full px-4 py-3 bg-white/70 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-300 focus:border-transparent outline-none transition-all resize-none text-sm"
                      rows={2} placeholder={{
    student: '如：英语提高30分',
    teacher: '如：减少备课时间',
    principal: '如：提升全校平均分',
    ...
    professional: '如：三年内成为技术 leader',
                ...
                <div className="flex gap-3 mt-5">
                  ...
                  <button type="button" onClick={handleGenerate} disabled={loading}
                    className="flex-1 py-3.5 btn-gradient-primary rounded-xl font-bold hover:shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
            ...
            {step === 3 && roleConfig && generated && (
              <div className="glass-card !p-6 rounded-2xl">
                ...
                <div className={`${roleConfig.lightBg} rounded-2xl p-6 mb-5 border border-white/50`}>
                ...
                <button type="button" onClick={handleRegister} disabled={loading}
                  className="w-full py-3.5 btn-gradient-primary rounded-xl font-bold text-base hover:shadow-lg disabled:opacity-50">
                  {loading ? '注册中...' : '确认并注册 →'}
                </button>
              </div>
            )}
        ...
        {activeTab === 'human' && step === 0 && (
          <>
            <div className="text-center">
              <button onClick={() => { loginAsGuest(); navigate('/'); }} className="px-6 py-2.5 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 text-sm font-medium hover:border-emerald-300 hover:text-emerald-600 hover:bg-emerald-50/50 transition-all">先逛逛，稍后再来 →</button>
            </div>
